...
The CLI is a capture sink whose primary input is free-form text, which legitimately begins with `-` (code, shell examples). We decided that only an exact match at the first position against a small allow-list (`-h`, `-v`, `--setup`, and future dash-prefixed commands like `--search`) is interpreted as a command; everything else, including dash-prefixed text, is note content joined by a single space. In the common POSIX alternative (clap default), any `-` token is a flag and unknown flags error, which would break core use cases like `bd git checkout -f` or `bd call -h support`.
